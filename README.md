...
CUDA Toolkit은 [여기](https://developer.nvidia.com/cuda-toolkit-archive)에서 다운로드 받을 수 있어요. 반드시 12.6.x 버전으로 받아주세요.
...
6. /jiaunloadmodel                  -  이 커맨드를 사용하면 현재 로드된 LLM 모델을 언로드해요.
7. /jiaping                         -  이 커맨드를 사용하면 지아가 "pong!" 메시지를 보내요.
8. /jiasay [문장]                   -  이 커맨드를 사용하면 지아가 [문장] 부분을 읽어줘요.
9. /jiahear (채널ID)                -  이 커맨드를 사용하면 지아가 듣는 내용을 (채널ID)에 적어줘요. (채널ID)를 입력하지 않으면 debug text channel에 적어줘요.
10. /jiastop                        -  이 커맨드를 사용하면 현재 지아가 재생중인 음성을 멈춰요.
11. /jiajoinnoagent                 -  이 커맨드를 사용하면 지아가 통화방에 들어와요. 하지만, 아무 기능도 작동하지 않아요.
12. /jiatalk                        -  이 커맨드를 사용하면 해당 채널에서 작성하는 모든 대화는 지아가 대답해줘요. 이제 /jia나 /지아를 입력하지 않아도 괜찮아요.
13. /jiastoptalk                    -  이 커맨드를 사용하면 /jiatalk 기능을 멈춰요.
