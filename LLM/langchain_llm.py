...
def _build_sys_prompt() -> str:
    ...
    if is_comfyui_enabled():
        prompt += "\n사용자가 그림이나 이미지를 그려달라고 하면 'generate_image' 도구를 호출해줘. prompt에는 영어 프롬프트를, wait_message에는 그리는 동안 채널에 먼저 보여줄 짧은 안내 문구를 너의 말투로 넣어줘. 안내 문구가 먼저 올라가고 생성이 끝나면 그 메시지가 그림으로 바뀌니, 너는 도구 호출 결과를 받은 뒤에 그림이 완성됐다고 자연스럽게 알려주면 돼."
    return prompt
