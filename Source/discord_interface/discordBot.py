...
from memory.RAG import sync_all_metadata_to_faiss
import requests
...
class ResultHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        if event.is_directory:
            return
        self.voice_client.play(discord.FFmpegPCMAudio(event.src_path), after=lambda e: os.remove(event.src_path) if e is None else logger.error(f"음성 재생 중 오류 발생: {e}"))
...
def bot():
    try:
        ...
        @app.post("/text-send")
        async def task_processor(requested: Task):
            ...
            try:
                resp = requests.get(f"http://localhost:8000/get-text-process/{task_id}")
                if resp.status_code == 200:
                    ...
                    fut = asyncio.run_coroutine_threadsafe(
                        ctx.send(f"결과를 가져오는 데 실패했어요. (status: {resp.status_code})"), bot.loop
                    )
                    fut.result()
        ...
        async def jiajoin(ctx):
            ...
            if ctx.author.voice is None:
                ...
                return
            sync_all_metadata_to_faiss(ctx.channel.guild.id)
            voice_channel = ctx.author.voice.channel
            voice_client = await voice_channel.connect(cls=voice_recv.VoiceRecvClient)
            event_handler = ResultHandler(voice_client)
            bot.observer = Observer()
            bot.observer.schedule(event_handler, path="Source/output_temp", recursive=False)
            bot.observer.start()
        ...
        async def jiareload(ctx):
            try:
                ...
                sync_all_metadata_to_faiss(ctx.guild.id)  # FAISS 인덱스 동기화
        ...
        async def jiaaddslang(ctx, word: str, meaning: str, examples: str):
            try:
                from memory.RAG import save_slang_metadata
                save_slang_metadata(word, meaning, examples, ctx.author.name, datetime.now().isoformat(), ctx.guild.id)
